...
public class LookupAsrRequestsController implements LookupAsrRequestsApi {
  ...
  @Override
  public ResponseEntity<AsrRequests> getAsrRequests(@ApiParam(required=true) @PathVariable("remoteStorageConfigurationId") String remoteStorageConfigurationId) {
    ...
    try {
      ...
      asrRequests.getAsrRequests().forEach(x -> remoteStorageService.setRetrievedAsync(x.getItemBarcode()));
    }
  }
}
